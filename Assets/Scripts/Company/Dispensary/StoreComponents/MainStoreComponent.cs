...
[System.Serializable]
public class MainStoreComponent : MonoBehaviour
{
    public DispensaryManager dm;
    public Database db;
    public OutdoorNode sidewalkLink; // The main store is the only component with a sidewalk link (might make multiple sidewalk linkable components later[maybe])
    ...
    public List<StoreObjectFunction_DisplayShelf> GetDisplayShelves()
    {
    ...
    public List<CheckoutCounter> GetCheckoutCounters()
    {
        List<CheckoutCounter> toReturn = new List<CheckoutCounter>();
        foreach (MonoBehaviour mono in storeObjects)
        {
            if (mono.GetType() == typeof(CheckoutCounter))
            {
                toReturn.Add((CheckoutCounter)mono);
            }
        }
        return toReturn;
    }

    public List<BudtenderCounter> GetBudtenderCounters()
    {
        List<BudtenderCounter> toReturn = new List<BudtenderCounter>();
        foreach (MonoBehaviour mono in storeObjects)
        {
            if (mono.GetType() == typeof(BudtenderCounter))
            {
                toReturn.Add((BudtenderCounter)mono);
            }
        }
        return toReturn;
    }
