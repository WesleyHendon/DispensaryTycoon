...
[Serializable]
public class HallwayComponent : MonoBehaviour
{
	// components - general
                ...
                catch (NullReferenceException)
                {
                    if (subGrid != null)
                    {
                        ...
                        for (int x = 0; x < subGrid.gridSizeX; x++)
                        {
                            for (int y = 0; y < subGrid.gridSizeY; y++)
                            {
                                gridIDs.extWallTileIDs[x, y] = 12002;
                            }
                        }
                    }
                }
	...
	public HallwayComponent_s MakeSerializable()
    {
        ...
        HallwayComponent_s toReturn = new HallwayComponent_s(index, gameObject.transform.position, gameObject.transform.eulerAngles.y, serializableGrids, serializableStoreObjects, serializableWindows);
        return toReturn;
	}
